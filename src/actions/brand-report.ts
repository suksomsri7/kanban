...
export async function getBrandOverdueCards(brandId: string) {
  await requireAuth();
...
export async function getBrandTeamPerformance(brandId: string) {
  await requireAuth();

  const brandMemberUserIds = await prisma.brandMember
    .findMany({
      where: { brandId },
      select: { userId: true },
    })
  ...
  if (brandMemberUserIds.length === 0) {
    return [];
  }

  const users = await prisma.user.findMany({
    where: {
      id: { in: brandMemberUserIds },
      isActive: true,
      role: { not: "GUEST" },
    },
    select: {
...
export async function getBrandBoardSummaries(brandId: string) {
  await requireAuth();
...
export async function getBrandExportData(brandId: string) {
  await requireAuth();
